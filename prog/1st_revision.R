...
df_load <- read_csv('data/scenario_data.csv') %>% 
  filter(Model!='MESSAGEix-MACRO-GLOBIOM') %>% 
...
df_load_w_MACRO <- read_csv('data/scenario_data.csv') %>% 
  pivot_longer(cols=-c(Model,Region,Scenario,Variable,Unit),
               names_to='Year',values_to='value',names_transform=as.numeric) %>% 
  mutate(Model=recode(Model,'AIM-Technology'='AIM','MESSAGEix-GLOBIOM'='MESSAGEix','MESSAGEix-MACRO-GLOBIOM'='MESSAGEix-MACRO')) %>% 
  drop_na(value)

df_load_R5 <- read_csv('data/scenario_data_R5.csv') %>% 
  filter(Model!='MESSAGEix-MACRO-GLOBIOM') %>% 
  pivot_longer(cols=-c(Model,Region,Scenario,Variable,Unit),
               names_to='Year',values_to='value',names_transform=as.numeric) %>% 
  mutate(Model=recode(Model,'AIM-Technology'='AIM','MESSAGEix-GLOBIOM'='MESSAGEix')) %>% 
...
# load IEA energy balance

if (plot_iea=='on') {
  df_load_IEA <- read_csv('data/IEAEB.csv')
}
...
df_fig_1a_annotation <- bezier_tern(c(0.4, 0.15, 0.35),
                                    c(0.225, 0.35, 0.42),
                                    c(0.05, 0.3, 0.695),
                                    n = 100,
                                    colnames = c("Fossil", "Biomass", "Nuclear + Non-biomass renewables"))

if (plot_iea=='on') {
df_fig_1a_IEA <- df_load_IEA %>% 
  filter(Year==2020,Region=='World',
         str_detect(Variable,'Primary Energy')) %>%
  pivot_wider(names_from=Variable,values_from=value) %>% 
  transmute(Region,Year,
            Fossil=(`Primary Energy|Coal`+`Primary Energy|Oil`+`Primary Energy|Gas`)/`Primary Energy`,
            Biomass=`Primary Energy|Biomass`/`Primary Energy`,
            `Nuclear + Non-biomass renewables`=(`Primary Energy`-(`Primary Energy|Coal`+`Primary Energy|Oil`+`Primary Energy|Gas`+`Primary Energy|Biomass`))/`Primary Energy`)
}
...
g_fig_1a <- df_fig_1a %>% 
  ...
  geom_mask() +
  geom_point(data=df_fig_1a_AR6 %>% filter(Year==2050,Category%in%c('C1','C2')),color='#D9D5C4FF',size=0.7,shape=4,stroke=0.4) +
  geom_point(aes(color=Model,shape=Scenario,size=Scenario,stroke=Scenario)) +
  ...
  annotate("text", x = 0.675, y = 0.075, z = 0.25, hjust=0.1,
           ...
           size = 1.75) +
  geom_path(data=df_fig_1a_annotation,
            linewidth = 0.275,color='grey40',arrow = arrow(length = unit(1, "mm"))) +
  annotate("text", x = 0.3125, y = 0.375, z = 0.3125, hjust=0.1,vjust=0.25,
           label='Fossil phase-out',size = 1.75) +
  annotate("text", x = 0.325, y = 0.35, z = 0.325, hjust=0.1,vjust=1,
           label=' (Opt1.5C \u2192 ZF2050)',size = 1.25) +
  geom_path(aes(color=Model),linewidth=0.25) +
  scale_color_manual(values=c('AIM'='#DD4477FF','MESSAGEix'='#0099C6FF')) +
  scale_shape_manual(values=c('Opt1.5C'=4,'ZF2100'=2,'ZF2050'=21,
                              'ZF2060'=0,'ZF2070'=8,'ZF2080'=5,'ZF2090'=3)) +
  scale_size_manual(values=c('Opt1.5C'=1.5,'ZF2100'=0.9,'ZF2050'=1.5,
                             'ZF2060'=0.9,'ZF2070'=0.9,'ZF2080'=0.9,'ZF2090'=0.9)) +
  scale_discrete_manual(
    aesthetics='stroke',
    values=c('Opt1.5C'=0.6,'ZF2100'=0.3,'ZF2050'=0.6,
             'ZF2060'=0.3,'ZF2070'=0.3,'ZF2080'=0.3,'ZF2090'=0.3)
  ) +
  labs(
    x='',y='',z='',
    Tarrow='Biomass(%)',
...
if (plot_iea=='on') {
  g_fig_1a <- g_fig_1a +
    geom_point(data=df_fig_1a_IEA,color='black',size=1.5,shape=13,stroke=0.3)
}
...
df_fig_1b_annotation <- bezier_tern(c(0.175, 0.05, 0.725),
                                      c(0.075, 0.125, 0.8),
                                      c(0.025, 0.05, 0.925),
                                      n=100,
                                      colnames = c("Fossil", "Biomass", "Nuclear + Non-biomass renewables"))
...
if (plot_iea=='on') {
  df_fig_1b_IEA <- df_load_IEA %>% 
    filter(Year==2020,Region=='World',
           str_detect(Variable,'Secondary Energy')) %>%
    pivot_wider(names_from=Variable,values_from=value) %>% 
    transmute(Region,Year,
              Fossil=`Secondary Energy|Electricity|Fossil`/`Secondary Energy|Electricity`,
              `Biomass`=`Secondary Energy|Electricity|Biomass`/`Secondary Energy|Electricity`,
              `Nuclear + Non-biomass renewables`=(`Secondary Energy|Electricity|Nuclear`+`Secondary Energy|Electricity|Non-Biomass Renewables`)/`Secondary Energy|Electricity`)
}
...
g_fig_1b <- df_fig_1b %>% 
  ...
  ggtern(aes(Fossil,`Biomass`,`Nuclear + Non-biomass renewables`)) +
  geom_mask() +
  geom_point(data=df_fig_1b_AR6 %>% filter(Year==2050,Category%in%c('C1','C2')),color='#D9D5C4FF',size=0.7,shape=4,stroke=0.4) +
  geom_point(aes(color=Model,shape=Scenario,size=Scenario,stroke=Scenario)) +
  annotate("text", x = 0.85, y = 0.075, z = 0.075, hjust=0.1,
  ...
  annotate("text", x = 0.675, y = 0.075, z = 0.25, hjust=0.1,
           ...
           size = 1.75) +
  geom_path(data=df_fig_1b_annotation,
            linewidth = 0.275,color='grey40',arrow = arrow(length = unit(1, "mm"))) +
  annotate("text", x = 0.25, y = 0.125, z = 0.625, hjust=0.1,
           label='Fossil phase-out',
           size = 1.75) +
  geom_path(aes(color=Model),linewidth=0.25) +
  scale_color_manual(values=c('AIM'='#DD4477FF','MESSAGEix'='#0099C6FF')) +
  scale_shape_manual(values=c('Opt1.5C'=4,'ZF2100'=2,'ZF2050'=21,
...
if (plot_iea=='on') {
g_fig_1b <- g_fig_1b +
    geom_point(data=df_fig_1b_IEA,color='black',size=1.5,shape=13,stroke=0.3)
}
...
df_fig_1c_annotation <- bezier_tern(c(0.2, 0.3, 0.5),
                                      c(0.225, 0.125, 0.65),
                                      c(0.1, 0.1, 0.8),
                                      n=100,
                                      colnames = c("Solids", "Liquid & gases", "Electricity, hydrogen & others"))

if (plot_iea=='on') {
  df_fig_1c_IEA <- df_load_IEA %>% 
    filter(Year==2020,Region=='World',
           str_detect(Variable,'Final Energy')) %>%
    pivot_wider(names_from=Variable,values_from=value) %>% 
    transmute(Region,Year,
              Solids=`Final Energy|Solids`/`Final Energy`,
              `Liquid & gases`=(`Final Energy|Liquids`+`Final Energy|Gases`)/`Final Energy`,
...
g_fig_1c <- df_fig_1c %>%
  ...
  geom_mask() +
  geom_point(data=df_fig_1c_AR6 %>% filter(Year==2050,Category%in%c('C1','C2')),color='#D9D5C4FF',size=0.7,shape=4,stroke=0.4) +
  geom_point(aes(color=Model,shape=Scenario,size=Scenario,stroke=Scenario)) +
  annotate("text", x = 0.85, y = 0.075, z = 0.075, hjust=0.1,
  ...
  annotate("text", x = 0.675, y = 0.075, z = 0.25, hjust=0.1,
           ...
           size = 1.75) +
  geom_path(data=df_fig_1c_annotation,
            linewidth = 0.275,color='grey40',arrow = arrow(length = unit(1, "mm"))) +
  annotate("text", x = 0.425, y = 0.15, z = 0.425, hjust=0.1,
           label='Fossil phase-out',
           size = 1.75) +
  geom_path(aes(color=Model),linewidth=0.25) +
  scale_color_manual(values=c('AIM'='#DD4477FF','MESSAGEix'='#0099C6FF')) +
...
if (plot_iea=='on') {
  g_fig_1c <- g_fig_1c +
    geom_point(data=df_fig_1c_IEA,color='black',size=1.5,shape=13,stroke=0.3)
}
...
g_fig_1d <- df_fig_1d %>% 
  ...
  geom_crossbar(data=df_fig_1d_AR6,
                aes(x=Year,ymin=p10,ymax=p90,y=p50),width=4.5,color='white',fill='#D9D5C4FF',linewidth=0.25)+
  geom_crossbar(data=df_fig_1d_AR6,
                aes(x=Year,ymin=p0,ymax=p100,y=p0),width=4.5,color='#D9D5C4FF',fill='transparent',fatten=0,linewidth=0.25)+
...
g_fig_1e <- df_fig_1e %>% 
  ...
  geom_crossbar(data=df_fig_1e_AR6,
                aes(x=Year,ymin=p10/1000,ymax=p90/1000,y=p50/1000),width=4.5,color='white',fill='#D9D5C4FF',linewidth=0.25)+
  geom_crossbar(data=df_fig_1e_AR6,
                aes(x=Year,ymin=p0/1000,ymax=p100/1000,y=p0/1000),width=4.5,color='#D9D5C4FF',fill='transparent',fatten=0,linewidth=0.25)+
...
l_fig_1abc_2 <- tribble(~y,~y2,~label,
  ...
  scale_color_manual(values=c('#D9D5C4FF','black')) +
...
l_fig_1de <- tribble(~y,~y2,~label,
  ...
  mutate(x=0) %>% 
  ggplot()+
  geom_crossbar(aes(x=x),ymin=.2,ymax=.8,y=.5,width=.15,linewidth=.15,color='white',fill='#D9D5C4FF')+
  geom_crossbar(aes(x=x),ymin=0,ymax=1,y=0,width=.15,linewidth=.15,color='#D9D5C4FF',fill='transparent',fatten=0)+
  geom_point(aes(y=y2),x=.18,shape=95,color='#D9D5C4FF',size=1.5)+
...
g_fig_1 <-ggdraw() +
  ...
  draw_text('(2050)',x=0.04,y=.91,size=7,hjust=-0.1) +
  draw_text('Power generation mix',x=.315,y=.94,size=7,hjust=-0.1) +
  draw_text('(2050)',x=0.365,y=.91,size=7,hjust=-0.1) +
  draw_text('Final energy mix',x=.675,y=.94,size=7,hjust=-0.1) +
  draw_text('(2050)',x=0.715,y=.91,size=7,hjust=-0.1) +
  draw_plot_label(
    label=c('a','b','c','d','e','f'),
...
ggplot2::ggsave('output/revision_1/fig_1.png',g_fig_1,width=180,height=105,units='mm',dpi=500,create.dir=TRUE)
...
g_fig_2b <- df_fig_2b %>%
  ...
  geom_crossbar(data=df_fig_2b_AR6,
                aes(x=Year,ymin=p10,ymax=p90,y=p50),width=4,color='white',fill='#D9D5C4FF',linewidth=0.25)+
  geom_crossbar(data=df_fig_2b_AR6,
                aes(x=Year,ymin=p0,ymax=p100,y=p0),width=4,color='#D9D5C4FF',fill='transparent',fatten=0,linewidth=0.25)+
  geom_line(aes(x=Year,y=value,color=Scenario,linetype=Scenario),linewidth=0.2) +
  geom_point(aes(x=Year,y=value,color=Scenario,shape=Scenario),size=0.6,stroke=0.3,fill=NA) +
  geom_text(data=df_fig_2b_AR6 %>% filter(Year==2050),
            aes(label=paste0('n=',n)),x=2025,y=0.825,hjust=0,vjust=0,size=1.75) +
  scale_color_manual(values=c('Opt1.5C'='grey35','ZF2100'='#FF9900FF','ZF2050'='#1616A7FF')) +
...
ggplot2::ggsave('output/revision_1/fig_2.png',g_fig_2,width=180,height=70,units='mm',dpi=1000)
...
df_fig_3c_AR6_1 <- df_load_AR6 %>% 
  filter(Variable%in%c('Secondary Energy|Electricity'),Year%in%c(2050,2100)) %>% 
  group_by(Region,Category,Case,Year) %>% 
  reframe(value=sum(value)) %>% 
  group_by(Region,Year) %>% 
  reframe(p50=median(value),p90=quantile(value,.9),p10=quantile(value,.1),p0=min(value),p100=max(value),n=n(),
...
df_fig_3c_AR6_2 <- df_load_AR6 %>% 
  filter(Variable%in%c('Secondary Energy|Hydrogen'),Year%in%c(2050,2100)) %>% 
  group_by(Region,Category,Case,Year) %>% 
  reframe(value=sum(value)) %>% 
  group_by(Region,Year) %>% 
  reframe(p50=median(value),p90=quantile(value,.9),p10=quantile(value,.1),p0=min(value),p100=max(value),n=n(),
...
g_fig_3c <- df_fig_3c %>% 
  filter(Year%in%c(2050,2100),Scenario%in%c('Opt1.5C','ZF2100','ZF2050')) %>% 
  ...
  geom_crossbar(data=df_fig_3c_AR6 %>% mutate(Year=as.character(Year)),
                aes(x=Year,ymin=p10,ymax=p90,y=p50),width=.5,color='white',fill='#D9D5C4FF',linewidth=0.3)+
  geom_crossbar(data=df_fig_3c_AR6 %>% mutate(Year=as.character(Year)),
                aes(x=Year,ymin=p0,ymax=p100,y=p0),width=.5,color='#D9D5C4FF',fill='transparent',fatten=0,linewidth=0.3)+
  geom_jitter(aes(x=Year,y=value,shape=Scenario,color=Model,size=Scenario),stroke=0.45,width=0) +
  labs(x='',y=expression(paste('Secondary energy (EJ ',{yr^{-1}},')'))) +
  geom_text(data=df_fig_3c_AR6 %>% mutate(Year=as.character(Year)),
            aes(x='2100',y=-20,label=paste0('n=',n)),size=1.75) +
  scale_color_manual(values=c('AIM'='#DD4477FF','MESSAGEix'='#0099C6FF')) +
...
df_fig_3d <- df_load %>%
  filter(Variable%in%c('Capacity Additions|Electricity|Solar|PV',
  ...
  mutate(Variable=recode(Variable,
                         'Capacity Additions|Electricity|Solar|PV'='Solar PV',
                         'Capacity Additions|Electricity|Wind'='Wind',
                         'Capacity Additions|Electricity|Storage Capacity'='Storage',
                         'Capacity Additions|Hydrogen|Electricity'='Electrolyzer'),
         Variable=factor(Variable,levels=c('Solar PV','Wind','Storage','Electrolyzer'))) %>% 
  filter(Scenario%in%c('Opt1.5C','ZF2050','ZF2100'),
         Year%in%seq(2025,2100,5)) %>% 
  mutate(Year10=paste0((Year-1)%/%10*10+1,'-',((Year-1)%/%10*10+10)-2000)) %>% 
  group_by(Model,Region,Scenario,Variable,Unit,Year10) %>% 
  reframe(value=mean(value))

g_fig_3d <- df_fig_3d %>% 
  ggplot() +
  geom_hline(yintercept=0,linewidth=0.1,color='grey15',linetype='dashed') +
  geom_line(aes(x=Year10,y=value/1000,color=Scenario,linetype=Scenario,group=Scenario),linewidth=0.2) +
  geom_area(aes(x=Year10,y=value/1000,fill=Scenario,group=Scenario),alpha=0.1,position='identity') +
  geom_point(aes(x=Year10,y=value/1000,color=Scenario,shape=Scenario,group=Scenario),size=1,stroke=0.5,fill=NA) +
  scale_color_manual(values=c('Opt1.5C'='grey35','ZF2100'='#FF9900FF','ZF2050'='#1616A7FF')) +
  scale_fill_manual(values=c('Opt1.5C'='grey35','ZF2100'='#FF9900FF','ZF2050'='#1616A7FF')) +
  scale_linetype_manual(values=c('Opt1.5C'='dotdash','ZF2100'='solid','ZF2050'='solid')) +
  scale_shape_manual(values=c('Opt1.5C'=4,'ZF2100'=2,'ZF2050'=21)) +
  scale_x_discrete(labels=c('2021-30',
                            '',
                            '2041-50',
                            '',
                            '2061-70',
                            ...
                            '2081-90',
                            '')) +
  labs(x='',y=expression(paste('Capacity addition (TW ',{yr^{-1}},')'))) +
...
g_fig_3 <-ggdraw() +
  ...
  draw_plot(g_fig_3c+theme(legend.position='none'),x=0,y=0,width=.2,height=.55) +
  ...
  draw_plot(l_fig_3c_1,x=.22,y=0.37,width=.1,height=.1) +
  draw_plot(l_fig_3c_2,x=.21,y=0.26,width=.1,height=.1) +
  draw_plot(l_fig_1de,x=.22,y=0.04,width=.075,height=.2) +
  draw_plot_label(
    label=c('a','b','c','d'),
    x=c(0,.45,0,0.36),
    y=c(1,1,.55,.55),
...
ggplot2::ggsave('output/revision_1/fig_3.png',g_fig_3,width=180,height=120,units='mm',dpi=500)
...
g_fig_4a <- df_fig_4a %>%
  ...
  geom_crossbar(data=df_fig_4a_AR6,
                aes(x=Year,ymin=p10/1000,ymax=p90/1000,y=p50/1000),width=3,color='white',fill='#D9D5C4FF',linewidth=0.25)+
  geom_crossbar(data=df_fig_4a_AR6,
                aes(x=Year,ymin=p0/1000,ymax=p100/1000,y=p0/1000),width=3,color='#D9D5C4FF',fill='transparent',fatten=0,linewidth=0.25)+
...
ggplot2::ggsave('output/revision_1/fig_4.png',g_fig_4,width=180,height=65,units='mm',dpi=500)
...
g_fig_5a  <- df_fig_5a %>% 
  ...
  scale_fill_manual(values=c('AIM'='#DD4477FF','MESSAGEix'='#0099C6FF')) +
  scale_shape_manual(values=c('Opt1.5C'=4,'ZF2100'=2,'ZF2050'=21,
                              'ZF2060'=0,'ZF2070'=8,'ZF2080'=5,'ZF2090'=3)) +
...
l_fig_5_2 <- ggplot() +
  geom_crossbar(aes(x=1,xmin=1,xmax=2,y=0),fill='#DD4477FF',width=0.4,color='black',fatten=0,linewidth=0.15,alpha=0.7)+
  geom_crossbar(aes(x=5,xmin=5,xmax=6,y=0),fill='#0099C6FF',width=0.4,color='black',fatten=0,linewidth=0.15,alpha=0.7)+
...
ggplot2::ggsave('output/revision_1/fig_5.png',g_fig_5,width=90,height=150,units='mm',dpi=500)
...
g_fig_S1a <- df_fig_1a %>% 
  ...
  ggtern(aes(Fossil,`Biomass`,`Nuclear + Non-biomass renewables`)) +
  geom_mask() +
  geom_point(data=df_fig_1a_AR6 %>% filter(Year==2100,Category%in%c('C1','C2')),color='#D9D5C4FF',size=0.7,shape=4,stroke=0.4) +
  geom_point(aes(color=Model,shape=Scenario,size=Scenario,stroke=Scenario)) +
  annotate("text", x = 0.85, y = 0.075, z = 0.075, hjust=0.1,
...
g_fig_S1b <- df_fig_1b %>% 
  ...
  ggtern(aes(Fossil,`Biomass`,`Nuclear + Non-biomass renewables`)) +
  geom_mask() +
  geom_point(data=df_fig_1b_AR6 %>% filter(Year==2100,Category%in%c('C1','C2')),color='#D9D5C4FF',size=0.7,shape=4,stroke=0.4) +
  geom_point(aes(color=Model,shape=Scenario,size=Scenario,stroke=Scenario)) +
  annotate("text", x = 0.85, y = 0.075, z = 0.075, hjust=0.1,
...
g_fig_S1c <- df_fig_1c %>%
  ...
  ggtern(aes(Solids,`Liquid & gases`,`Electricity, hydrogen & others`)) +
  geom_mask() +
  geom_point(data=df_fig_1c_AR6 %>% filter(Year==2100,Category%in%c('C1','C2')),color='#D9D5C4FF',size=0.7,shape=4,stroke=0.4) +
  geom_point(aes(color=Model,shape=Scenario,size=Scenario,stroke=Scenario)) +
  annotate("text", x = 0.85, y = 0.075, z = 0.075, hjust=0.1,
...
ggplot2::ggsave('output/revision_1/fig_S1.png',g_fig_S1,width=180,height=105,units='mm',dpi=500,create.dir=TRUE)
...
ggplot2::ggsave('output/revision_1/fig_S2.png',g_fig_S2,width=180,height=110,units='mm',dpi=500,create.dir=TRUE)
...
g_fig_S3 <-ggdraw() +
  ...
  theme(panel.background=element_rect(fill='white',colour=NA))
plot(g_fig_S3)
ggplot2::ggsave('output/revision_1/fig_S3.png',g_fig_S3,width=180,height=60,units='mm',dpi=500,create.dir=TRUE)
...
ggplot2::ggsave('output/revision_1/fig_S4.png',g_fig_S4,width=180,height=120,units='mm',dpi=500,create.dir=TRUE)
...
g_fig_S5a <- df_fig_4b %>% 
  ...
  geom_point(data=. %>% group_by(Model,Scenario,Region,Unit,Year) %>% reframe(value=sum(value)),
             aes(x=Year,y=value/1000),size=.9,color='black',fill='white',stroke=0.6,shape=21) +
  scale_fill_manual(values=plt$Color,labels=plt$Legend) +
  labs(x='',y=expression(paste({CO[2]},' emission (Gt-',{CO[2]},' ',{yr^{-1}},')'))) +
  facet_grid(rows=vars(Model),cols=vars(Scenario),
             axes='all',axis.labels='margins') +
...
ggplot2::ggsave('output/revision_1/fig_S5.png',g_figS5,width=180,height=150,units='mm',dpi=500,create.dir=TRUE)
...
ggplot2::ggsave('output/revision_1/fig_S6.png',g_fig_S6,width=180,height=75,units='mm',dpi=500,create.dir=TRUE)


# fig. S7 -----------------------------------------------------------------

# import data
# ! please download from https://zenodo.org/records/17195662 & place it in the data directory
mineral_intensity <- read_xlsx('data/minerals/Supplementary Materials for Navigating energy transition solutions for climate targets with minerals constraint.xlsx','S5. Mineral_intensity') %>%
  drop_na(Value) %>% 
  transmute(Category=`Category technology`,
            Subdivision=case_when(`Category technology`=='wind'~'wind',TRUE~`Subdivision technology`),
            Metal=`Symbol `,
            Min=as.numeric(str_split(Value,"-",simplify=TRUE)[,1]),
            Max=as.numeric(str_split(Value,"-",simplify=TRUE)[,2]),
            Mean=`The average value \r\n(after some outliers have been deleted)`)
...
market_share <- read_xlsx('data/minerals/Supplementary Materials for Navigating energy transition solutions for climate targets with minerals constraint.xlsx',
                          'S11 Market share') %>% 
  transmute(Category=technology,
            Subdivision=abbreviation,
            Share=`market share assumptions`) %>%
  fill(Category,.direction='down') %>% 
  mutate(Category=case_when(Category=='Hydrogen by Electricity'~'Electric hydrogen',
                            Category=='Electricity Storage'~'Energy storage',
                            TRUE~Category)) %>%
  mutate(across(c(Category, Subdivision), norm_key))

map_iamc <- read_csv('data/minerals/iamc_map.csv',col_names=FALSE) %>% 
  rename(Variable=X1,Category=X2,Subdivision=X3) %>% 
  mutate(across(c(Category, Subdivision), norm_key)) %>% 
  full_join(market_share,by=c('Category','Subdivision')) %>% 
  replace_na(list(Share=1)) %>% 
  full_join(mineral_intensity) %>% 
  filter(Category!='DAC')

# main
df_mineral <- df_load %>% 
  mutate(Value=ifelse(Variable=='Capacity Additions|Electricity|Storage Capacity',value/6.2113,value)) %>% 
  inner_join(map_iamc) %>% 
  transmute(Model,Scenario,Variable,
            Year=as.numeric(Year),
            Category,Metal,
            Min=Min*value*Share,
            Max=Max*value*Share,
            Mean=Mean*value*Share)

df_fig_S7 <- df_mineral %>% 
  group_by(Model,Scenario,Year,Metal) %>% 
  reframe(Min=sum(Min),
          Max=sum(Max),
          Mean=sum(Mean)) %>% 
  group_by(Model,Scenario,Metal) %>% 
  complete(Year=seq(2020,2100)) %>% 
  mutate(Min=na_locf(Min,option='nocb'),
         Mean=na_locf(Mean,option='nocb'),
         Max=na_locf(Max,option='nocb')) %>% 
  group_by(Model,Scenario,Metal) %>% 
  mutate(Min=cumsum(Min),
         Mean=cumsum(Mean),
         Max=cumsum(Max)) %>% 
  filter(Year%in%seq(2020,2055,5)|Year%in%seq(2060,2100,5)&Model=='AIM'|Year%in%seq(2060,2100,10)&Model=='MESSAGEix')

g_fig_S7<- df_fig_S7 %>%
  filter(Scenario %in% c('Opt1.5C','ZF2050','ZF2100'),
         Metal %in% c('Cu','Li','Ni','Co','C','Nd','Pr','Dy','Tb')) %>%
  mutate(Metal2 = case_when(
    Metal=='Cu' ~ 'Copper',
    Metal=='Li' ~ 'Lithium',
    Metal=='Ni' ~ 'Nickel',
    Metal=='Co' ~ 'Cobalt',
    Metal=='C'  ~ 'Graphite',
    Metal %in% c('Nd','Pr','Dy','Tb') ~ 'Rare earth\nelements'
  )) %>%
  group_by(Model,Scenario,Metal2,Year) %>% 
  reframe(Min=sum(Min),Max=sum(Max),Mean=sum(Mean)) %>% 
  mutate(Metal2=factor(Metal2,levels=c('Copper','Lithium','Nickel','Cobalt','Graphite','Rare earth\nelements'))) %>% 
  ggplot() +
  geom_hline(yintercept=0, linewidth=0.1, color='grey15', linetype='dashed') +
  geom_line(aes(x = Year, y = Mean/10^6, color = Scenario,
                linetype = Scenario, group = Scenario), linewidth=0.2) +
  geom_point(aes(x = Year, y = Mean/10^6, color = Scenario,
                 shape = Scenario, group = Scenario), size=1, stroke=0.5, fill=NA) +
  scale_color_manual(values = c('Opt1.5C'='grey35','ZF2100'='#FF9900FF','ZF2050'='#1616A7FF')) +
  scale_linetype_manual(values = c('Opt1.5C'='dotdash','ZF2100'='solid','ZF2050'='solid')) +
  scale_shape_manual(values   = c('Opt1.5C'=4,'ZF2100'=2,'ZF2050'=21)) +
  labs(x='', y=expression(paste('Cumulative mineral demand (Mt)'))) +
  facet_wrap(vars(Model,Metal2), scales='free_y', axes='all', axis.labels='all_y', nrow=2) +
  plot_theme_white +
  theme(legend.position='right')
plot(g_fig_S7)

ggplot2::ggsave('output/revision_1/fig_S7.png',g_fig_S7,width=180,height=100,units='mm',dpi=500,create.dir=TRUE)
...
g_fig_S8a <- df_fig_S8 %>%
  ...
  ggplot() +
  geom_hline(yintercept=0,linewidth=0.1,color='grey15',linetype='dashed') +
  geom_crossbar(data=df_fig_S8_AR6,
                aes(x=Year,ymin=p10,ymax=p90,y=p50),width=2,color='white',fill='#D9D5C4FF',linewidth=0.25)+
  geom_crossbar(data=df_fig_S8_AR6,
                aes(x=Year,ymin=p0,ymax=p100,y=p0),width=2,color='#D9D5C4FF',fill='transparent',fatten=0,linewidth=0.25)+
  geom_line(aes(x=Year,y=value,color=Scenario,group=interaction(Model,Scenario),linetype=Scenario),linewidth=0.2) +
  geom_point(aes(x=Year,y=value,color=Scenario,shape=Scenario),size=0.8,stroke=0.4) +
  geom_text(data=df_fig_S8_AR6 %>% filter(Year==2050),
            aes(label=paste0('n=',n)),x=2025,y=-20,hjust=0,vjust=0,size=1.75) +
  geom_hline(yintercept=100,linewidth=0.15,color='grey15',linetype='dotted') +
  geom_hline(yintercept=245,linewidth=0.15,color='grey15',linetype='dotted') +
  geom_hline(yintercept=300,linewidth=0.15,color='grey15',linetype='dotted') +
  geom_text(label=expression(paste('100 EJ  ',{yr^{-1}})),x=2090,y=105,hjust=0,vjust=0,size=1.5) +
  geom_text(label=expression(paste('245 EJ  ',{yr^{-1}})),x=2090,y=250,hjust=0,vjust=0,size=1.5) +
  geom_text(label=expression(paste('300 EJ  ',{yr^{-1}})),x=2090,y=305,hjust=0,vjust=0,size=1.5) +
  scale_color_manual(values=c('Opt1.5C'='grey30',
                              'ZF2050'="#2621A8FF",
                              'ZF2060'="#5D03AEFF",
                              'ZF2070'="#9C179EFF",
  ...
  scale_shape_manual(values=c('Opt1.5C'=4,
                              ...
                              'ZF2090'=3)) +
  labs(x='',y=expression(paste('Primary Energy (EJ  ',{yr^{-1}},')'))) +
  facet_wrap(vars(Model),nrow=1,axes='all') +
  plot_theme_white +
  theme(legend.position='right')
plot(g_fig_S8a)

g_fig_S8 <-ggdraw() +
  draw_plot(g_fig_S8a+theme(legend.position='none'),x=0,y=0,width=.85,height=1) +
  draw_plot(l_fig_4a,x=.855,y=.55,width=.15,height=.4) +
  draw_plot(l_fig_1de,x=.89,y=.15,width=.1,height=.35) +
  theme(panel.background=element_rect(fill='white',colour=NA))
plot(g_fig_S8)

ggplot2::ggsave('output/revision_1/fig_S8.png',g_fig_S8,width=130,height=65,units='mm',dpi=500,create.dir=TRUE)


# fig.S9 ------------------------------------------------------------------

df_fig_S9 <- df_load_R5 %>%
  filter(Variable%in%c('Capacity Additions|Electricity|Solar|PV',
                       'Capacity Additions|Electricity|Wind',
                       'Capacity Additions|Hydrogen|Electricity',
                       'Capacity Additions|Electricity|Storage Capacity')) %>%
...
g_fig_S9_ASIA <- df_fig_S9 %>% 
  filter(Region=='R5ASIA') %>% 
  ggplot() +
  geom_hline(yintercept=0,linewidth=0.1,color='grey15',linetype='dashed') +
  geom_line(aes(x=Year10,y=value/1000,color=Scenario,linetype=Scenario,group=Scenario),linewidth=0.2) +
  geom_area(aes(x=Year10,y=value/1000,fill=Scenario,group=Scenario),alpha=0.1,position='identity') +
  ...
  scale_x_discrete(labels=c('2021-30',
                            ...
                            '')) +
  labs(x='',y=expression(paste('Capacity addition (TW ',{yr^{-1}},')'))) +
  facet_wrap(vars(Model,Variable),scales='free_y',axes='all',axis.labels='all_y',nrow=1) +
  plot_theme_white +
  theme(legend.position='right',
        axis.text.x=element_text(vjust=1,hjust=1))
...
g_fig_S9_ASIA <- df_fig_S9 %>% 
  filter(Region=='R5ASIA') %>% 
  mutate(Region=recode(Region,'R5ASIA'='Asia')) %>% 
  ggplot() +
  geom_hline(yintercept=0,linewidth=0.1,color='grey15',linetype='dashed') +
  geom_line(aes(x=Year10,y=value,color=Scenario,linetype=Scenario,group=Scenario),linewidth=0.2) +
  geom_area(aes(x=Year10,y=value,fill=Scenario,group=Scenario),alpha=0.1,position='identity') +
  geom_point(aes(x=Year10,y=value,color=Scenario,shape=Scenario,group=Scenario),size=1,stroke=0.5,fill=NA) +
  scale_color_manual(values=c('Opt1.5C'='grey35','ZF2100'='#FF9900FF','ZF2050'='#1616A7FF')) +
  scale_fill_manual(values=c('Opt1.5C'='grey35','ZF2100'='#FF9900FF','ZF2050'='#1616A7FF')) +
  scale_linetype_manual(values=c('Opt1.5C'='dotdash','ZF2100'='solid','ZF2050'='solid')) +
  scale_shape_manual(values=c('Opt1.5C'=4,'ZF2100'=2,'ZF2050'=21)) +
  scale_x_discrete(labels=c('2021-30',
                            ...
                            '2081-90',
                            '')) +
  labs(x='',y=expression(paste('Capacity addition (GW ',{yr^{-1}},')')),
       title='Asia') +
  facet_wrap(vars(Model,Variable),scales='free_y',axes='all',axis.labels='all_y',nrow=1) +
  plot_theme_white +
  theme(legend.position='right',
...
g_fig_S9_LAM <- df_fig_S9 %>% 
  filter(Region=='R5LAM') %>% 
  mutate(Region=recode(Region,'R5LAM'='Latin America and the Caribbean')) %>% 
  ggplot() +
  geom_hline(yintercept=0,linewidth=0.1,color='grey15',linetype='dashed') +
  geom_line(aes(x=Year10,y=value,color=Scenario,linetype=Scenario,group=Scenario),linewidth=0.2) +
  geom_area(aes(x=Year10,y=value,fill=Scenario,group=Scenario),alpha=0.1,position='identity') +
...
g_fig_S9_MAF <- df_fig_S9 %>% 
  filter(Region=='R5MAF') %>% 
  mutate(Region=recode(Region,'R5MAF'='Middle East and Africa')) %>% 
  ggplot() +
  geom_hline(yintercept=0,linewidth=0.1,color='grey15',linetype='dashed') +
  geom_line(aes(x=Year10,y=value,color=Scenario,linetype=Scenario,group=Scenario),linewidth=0.2) +
  geom_area(aes(x=Year10,y=value,fill=Scenario,group=Scenario),alpha=0.1,position='identity') +
...
g_fig_S9_OECD <- df_fig_S9 %>% 
  filter(Region=='R5OECD90+EU') %>% 
  mutate(Region=recode(Region,'R5OECD90+EU'='OECD 90 and EU')) %>% 
  ggplot() +
  geom_hline(yintercept=0,linewidth=0.1,color='grey15',linetype='dashed') +
  geom_line(aes(x=Year10,y=value,color=Scenario,linetype=Scenario,group=Scenario),linewidth=0.2) +
  geom_area(aes(x=Year10,y=value,fill=Scenario,group=Scenario),alpha=0.1,position='identity') +
...
g_fig_S9_REF <- df_fig_S9 %>% 
  filter(Region=='R5REF') %>% 
  mutate(Region=recode(Region,'R5REF'='Reforming Economies')) %>% 
  ggplot() +
  geom_hline(yintercept=0,linewidth=0.1,color='grey15',linetype='dashed') +
  geom_line(aes(x=Year10,y=value,color=Scenario,linetype=Scenario,group=Scenario),linewidth=0.2) +
  geom_area(aes(x=Year10,y=value,fill=Scenario,group=Scenario),alpha=0.1,position='identity') +
...
g_fig_S9 <- g_fig_S9_ASIA + labs(tag='a') +
  g_fig_S9_LAM + labs(tag='b') +
  g_fig_S9_MAF + labs(tag='c') +
  g_fig_S9_OECD + labs(tag='d') +
  g_fig_S9_REF + labs(tag='e') +
  plot_layout(ncol=1,guides='collect')&theme(legend.position='bottom')
plot(g_fig_S9)

ggplot2::ggsave('output/revision_1/fig_S9.png',g_fig_S9,width=180,height=240,units='mm',dpi=500,create.dir=TRUE)
...
g_fig_S10a <-df_load_w_MACRO %>% 
  filter(Variable%in%c('Final Energy'),Model!='AIM') %>% 
  pivot_wider(names_from=Scenario,values_from=value) %>% 
  mutate(across(c(Opt1.5C,ZF2050,ZF2060,ZF2070,ZF2080,ZF2090,ZF2100),~./Opt1.5C)) %>% 
  select(-Opt1.5C) %>% 
  pivot_longer(cols=c(ZF2050,ZF2060,ZF2070,ZF2080,ZF2090,ZF2100),names_to='Scenario',values_to='value') %>% 
  ggplot() +
  geom_hline(yintercept=1,linewidth=0.1,color='grey15',linetype='dashed') +
  geom_line(aes(x=Year,y=value,color=Scenario),linewidth=0.2) +
  geom_point(aes(x=Year,y=value,color=Scenario,shape=Scenario),size=0.8,stroke=0.4) +
  scale_color_manual(values=c('ZF2050'="#2621A8FF",
                              'ZF2060'="#5D03AEFF",
                              'ZF2070'="#9C179EFF",
                              'ZF2080'="#CC4678FF",
                              'ZF2090'="#ED7953FF",
                              'ZF2100'="#FDB32FFF")) +
  scale_shape_manual(values=c('ZF2100'=21,
                              'ZF2050'=2,
                              'ZF2060'=0,
                              'ZF2070'=8,
                              'ZF2080'=5,
  ...
  scale_y_continuous(labels=scales::percent) +
  guides(fill=guide_legend(ncol=1)) +
  labs(x='',y=expression(paste('Change in total final consumption (%)')),tag='a') +
  facet_wrap(vars(Model)) +
  plot_theme_white +
  theme(legend.position=c(0.4,0.3))
plot(g_fig_S10a)

g_fig_S10b <-df_load_w_MACRO %>% 
  filter(Variable%in%c('Final Energy|Solids|Coal',
                       'Final Energy|Liquids|Fossil',
                       'Final Energy|Solids|Biomass',
                       'Final Energy|Liquids|Biomass',
                       ...
                       'Final Energy|Liquids|Hydrogen synfuel'),
         Model!='AIM',
         Scenario%in%c('Opt1.5C','ZF2100','ZF2050'),
         Year%in%c(2050,2100)) %>% 
  mutate(Variable=case_when(str_detect(Variable,'Biomass')~'Final Energy|Biomass',
                            TRUE~Variable)) %>% 
  group_by(Model,Scenario,Region,Variable,Unit,Year) %>% 
  ...
  mutate(value=value/sum(value)) %>% 
  ggplot() +
  geom_bar(aes(x=Model,y=value,fill=Variable),stat='identity') +
  scale_fill_manual(values=plt$Color,labels=plt$Legend) +
  scale_y_continuous(labels=scales::percent) +
  guides(fill=guide_legend(ncol=1)) +
  labs(x='',y=expression(paste('Share in final energy (%)')),tag='b') +
  facet_wrap(vars(Year,Scenario),
             axes='all',axis.labels='margins',
             nrow=1) +
  plot_theme_white +
  theme(legend.position='right')
...
g_fig_S10 <- g_fig_S10a + g_fig_S10b + plot_layout(widths=c(1,1))
plot(g_fig_S10)

ggplot2::ggsave('output/revision_1/fig_S10.png',g_fig_S10,width=180,height=80,units='mm',dpi=500,create.dir=TRUE)


# Fig. S11 ----------------------------------------------------------------

df_fig_S11 <- read_csv(paste0('data/carbon_price.csv'),col_names=FALSE) %>% 
  transmute(Scenario=X1,Year=X5,value=X6*12/44)

g_fig_S11 <-df_fig_S11 %>% 
  ggplot() +
  geom_hline(yintercept=1,linewidth=0.1,color='grey15',linetype='dashed') +
  geom_line(aes(x=Year,y=value,color=Scenario,linetype=Scenario),linewidth=0.2) +
  geom_point(aes(x=Year,y=value,color=Scenario,shape=Scenario),size=0.8,stroke=0.4) +
  ...
  scale_shape_manual(values=c('Opt1.5C'=4,
                              ...
                              'ZF2090'=3)) +
  guides(fill=guide_legend(ncol=1)) +
  labs(x='',y=expression(paste('Carbon price (USD t-',{CO[2]}^{-1},')'))) +
  plot_theme_white +
  theme(legend.position=c(0.3,0.5))
plot(g_fig_S11)

ggplot2::ggsave('output/revision_1/fig_S11.png',g_fig_S11,width=90,height=70,units='mm',dpi=500,create.dir=TRUE)
