...
plot_iea <- 'on' # set to 'on' to include IEA energy balance data in the ternary plots (Fig. 1a-c)
...
df_load <- read_csv('data/scenario_data.csv') %>%
  ...
  pivot_longer(
    cols = -c(Model, Region, Scenario, Variable, Unit),
    names_to = 'Year',
    values_to = 'value',
...
df_load_w_MACRO <- read_csv('data/scenario_data.csv') %>%
  ...
  mutate(
    Model = recode(
      ...
      'AIM-Technology' = 'AIM',
      'MESSAGEix-GLOBIOM' = 'MESSAGEix',
      'MESSAGEix-MACRO-GLOBIOM' = 'MESSAGEix-MACRO'
    )
  ) %>%
  drop_na(value)
...
ls_Category <- read_xlsx(
  paste0('data/AR6_Scenarios_Database_metadata_indicators_v1.1.xlsx'),
...
# ! please download AR6 Scenarios Database world v1.1 & place it in the data directory
df_load_AR6 <- read_csv(paste0(
  'data/AR6_Scenarios_Database_World_v1.1.csv'
)) %>%
  ...
  pivot_longer(
    cols = -c(Case, Category, Region, Variable),
    names_to = 'Year',
    values_to = 'value',
    names_transform = as.numeric
  )
...
df_fig_1a <- df_load %>%
  ...
  mutate(across(
    c(`Fossil`, `Biomass`, `Nuclear + Non-biomass renewables`),
    ~ . / sum(`Fossil`, `Biomass`, `Nuclear + Non-biomass renewables`)
  ))
...
df_fig_1a_annotation <- bezier_tern(
  c(0.4, 0.15, 0.35),
  c(0.225, 0.35, 0.42),
  c(0.05, 0.3, 0.695),
  n = 100,
...
if (plot_iea == 'on') {
  df_fig_1a_IEA <- df_load_IEA %>%
    ...
    transmute(
      ...
      `Nuclear + Non-biomass renewables` = (`Primary Energy` -
        (`Primary Energy|Coal` +
          `Primary Energy|Oil` +
          `Primary Energy|Gas` +
          `Primary Energy|Biomass`)) /
...
g_fig_1a <- df_fig_1a %>%
  ...
  geom_mask() +
  geom_point(
    data = df_fig_1a_AR6 %>% filter(Year == 2050, Category %in% c('C1', 'C2')),
    color = '#D9D5C4FF',
    size = 0.7,
    shape = 4,
    stroke = 0.4
  ...
  annotate(
    ...
    label = paste0(
      "'C1: ' * italic(n)==",
      df_fig_1a_AR6 %>%
        filter(Year == 2050, Category %in% c('C1')) %>%
        nrow()
    ),
    parse = TRUE,
    size = 1.75
  ...
  annotate(
    ...
    label = paste0(
      "'C2: ' * italic(n)==",
      df_fig_1a_AR6 %>%
        filter(Year == 2050, Category %in% c('C2')) %>%
        nrow()
    ),
    parse = TRUE,
    size = 1.75
  ...
  annotate(
    ...
    hjust = 0.1,
    vjust = 1,
    label = ' (Opt1.5C \u2192 ZF2050)',
    size = 1.25
  ) +
  geom_path(aes(color = Model), linewidth = 0.25) +
  scale_color_manual(
    values = c('AIM' = '#DD4477FF', 'MESSAGEix' = '#0099C6FF')
  ) +
  scale_shape_manual(
    values = c(
      'Opt1.5C' = 4,
...
df_fig_1b <- df_load %>%
  ...
  transmute(
    ...
    Region,
    Year,
    Fossil = (`Secondary Energy|Electricity|Coal` +
      `Secondary Energy|Electricity|Oil` +
      `Secondary Energy|Electricity|Gas`) /
      `Secondary Energy|Electricity`,
    `Biomass` = (`Secondary Energy|Electricity|Biomass`) /
      `Secondary Energy|Electricity`,
    `Nuclear + Non-biomass renewables` = (`Secondary Energy|Electricity` -
      (`Secondary Energy|Electricity|Coal` +
        `Secondary Energy|Electricity|Oil` +
        `Secondary Energy|Electricity|Gas` +
        `Secondary Energy|Electricity|Biomass`)) /
...
df_fig_1b_annotation <- bezier_tern(
  c(0.175, 0.05, 0.725),
  c(0.075, 0.125, 0.8),
  c(0.025, 0.05, 0.925),
  n = 100,
...
if (plot_iea == 'on') {
  df_fig_1b_IEA <- df_load_IEA %>%
    ...
    transmute(
      ...
      `Biomass` = `Secondary Energy|Electricity|Biomass` /
        `Secondary Energy|Electricity`,
      `Nuclear + Non-biomass renewables` = (`Secondary Energy|Electricity|Nuclear` +
...
g_fig_1b <- df_fig_1b %>%
  ...
  geom_mask() +
  geom_point(
    data = df_fig_1b_AR6 %>% filter(Year == 2050, Category %in% c('C1', 'C2')),
    color = '#D9D5C4FF',
    size = 0.7,
    shape = 4,
    stroke = 0.4
...
df_fig_1c <- df_load %>%
  ...
  transmute(
    ...
    `Liquid & gases` = (`Final Energy|Liquids` + `Final Energy|Gases`) /
      `Final Energy`,
    `Electricity, hydrogen & others` = (`Final Energy` -
      (`Final Energy|Solids` + `Final Energy|Liquids` + `Final Energy|Gases`)) /
      `Final Energy`
  )
...
df_fig_1c_annotation <- bezier_tern(
  c(0.2, 0.3, 0.5),
  c(0.225, 0.125, 0.65),
  c(0.1, 0.1, 0.8),
  n = 100,
...
g_fig_1c <- df_fig_1c %>%
  ...
  geom_mask() +
  geom_point(
    data = df_fig_1c_AR6 %>% filter(Year == 2050, Category %in% c('C1', 'C2')),
    color = '#D9D5C4FF',
    size = 0.7,
    shape = 4,
    stroke = 0.4
...
df_fig_1d <- df_load %>%
  filter(
    Variable %in%
      c('Primary Energy|Coal', 'Primary Energy|Oil', 'Primary Energy|Gas')
  ) %>%
...
g_fig_1d <- df_fig_1d %>%
  ...
  geom_crossbar(
    data = df_fig_1d_AR6,
    aes(x = Year, ymin = p10, ymax = p90, y = p50),
    width = 4.5,
    color = 'white',
    fill = '#D9D5C4FF',
    linewidth = 0.25
  ...
  geom_crossbar(
    data = df_fig_1d_AR6,
    aes(x = Year, ymin = p0, ymax = p100, y = p0),
    width = 4.5,
    color = '#D9D5C4FF',
    fill = 'transparent',
    fatten = 0,
  ...
  geom_point(
    data = . %>%
      filter(
        Year %in%
          c(seq(2020, 2055, 5), seq(2060, 2100, 10)) &
          Model == 'MESSAGEix' |
          Year %in% seq(2020, 2100, 5) & Model == 'AIM'
      ),
    aes(x = Year, y = value, shape = Scenario, color = Model, size = Scenario),
    stroke = 0.4
  ) +
  annotate(
    "text",
...
g_fig_1e <- df_fig_1e %>%
  ...
  geom_crossbar(
    data = df_fig_1e_AR6,
    aes(x = Year, ymin = p10 / 1000, ymax = p90 / 1000, y = p50 / 1000),
    width = 4.5,
    color = 'white',
    fill = '#D9D5C4FF',
    linewidth = 0.25
  ...
  geom_crossbar(
    data = df_fig_1e_AR6,
    aes(x = Year, ymin = p0 / 1000, ymax = p100 / 1000, y = p0 / 1000),
    width = 4.5,
    color = '#D9D5C4FF',
    fill = 'transparent',
    fatten = 0,
...
df_fig_1f <- df_load %>%
  ...
  transmute(
    ...
    Unit,
    Year,
    `Primary Energy|Coal|w/ CCS` = `Primary Energy|Coal` -
      `Primary Energy|Coal|w/o CCS`,
    `Primary Energy|Oil|w/ CCS` = `Primary Energy|Oil` -
      `Primary Energy|Oil|w/o CCS`,
    `Primary Energy|Gas|w/ CCS` = `Primary Energy|Gas` -
      `Primary Energy|Gas|w/o CCS`,
    `Primary Energy|Coal|w/o CCS`,
    `Primary Energy|Oil|w/o CCS`,
    `Primary Energy|Gas|w/o CCS`
  ...
  pivot_longer(
    cols = -c(Model, Scenario, Region, Unit, Year),
    names_to = 'Variable',
    values_to = 'value'
  ) %>%
...
g_fig_1f <- df_fig_1f %>%
  filter(Scenario %in% c('Opt1.5C', 'ZF2100', 'ZF2050')) %>%
  mutate(
    Variable = factor(Variable, levels = rev(var_prm$Variable)),
  ...
  geom_bar(
    aes(x = Scenario, y = value / 1000, fill = Variable),
    stat = 'identity'
  ) +
...
l_fig_1abc_2 <- tribble(
  ...
  ylim(-.4, 2.3) +
  theme_void() +
  theme(plot.margin = unit(c(0, 0, 0, 0), 'mm'), legend.position = 'none')
...
ggplot2::ggsave(
  'output/revision_final/fig_1.png',
  g_fig_1,
  width = 180,
  height = 105,
  units = 'mm',
...
ggplot2::ggsave(
  'output/revision_final/fig_1.pdf',
  g_fig_1,
  width = 180,
  height = 105,
  units = 'mm',
...
df_fig_2a <- df_load %>%
  ...
  mutate(
    Variable = case_when(
      str_detect(Variable, 'Biomass') ~ 'Final Energy|Biomass',
      TRUE ~ Variable
    )
  ) %>%
...
df_fig_2b_AR6_hydcar <- df_load_AR6 %>%
  ...
  transmute(
    ...
    Fossil = (`Final Energy|Solids|Coal` +
      (`Secondary Energy|Liquids` - `Secondary Energy|Liquids|Biomass`) /
        `Secondary Energy|Liquids` *
        `Final Energy|Liquids` +
      (`Secondary Energy|Gases` - `Secondary Energy|Gases|Biomass`) /
        `Secondary Energy|Gases` *
        `Final Energy|Gases`) /
      `Final Energy`,
  ...
  pivot_longer(
    cols = -c(Region, Case, Category, Year),
    names_to = 'Variable',
    values_to = 'value'
  ) %>%
  filter(Year %in% c(2030, 2050, 2070, 2100)) %>%
...
df_fig_2b_AR6_hyd <- df_load_AR6 %>%
  filter(
    Category %in% c('C1', 'C2'),
    Year %in% c(2030, 2050, 2070, 2100),
    Variable %in% c('Final Energy', 'Final Energy|Hydrogen')
  ) %>%
  pivot_wider(names_from = Variable, values_from = value) %>%
...
df_fig_2b_AR6 <- bind_rows(
  df_fig_2b_AR6_hydcar,
  df_fig_2b_AR6_ele,
  df_fig_2b_AR6_hyd
) %>%
  ...
  mutate(
    Variable = factor(
      Variable,
      levels = c('Fossil', 'Electricity', 'Biomass', 'Hydrogen', 'Synfuel')
    )
  )
...
g_fig_2b <- df_fig_2b %>%
  ...
  geom_point(
    aes(x = Year, y = value, color = Scenario, shape = Scenario),
    size = 0.6,
    stroke = 0.3,
    fill = NA
  ) +
  geom_text(
    data = df_fig_2b_AR6 %>% filter(Year == 2050),
    aes(label = paste0("italic(n)==", n)),
    parse = TRUE,
    x = 2025,
    y = 0.825,
    hjust = 0,
  ...
  scale_linetype_manual(
    values = c('Opt1.5C' = 'dotdash', 'ZF2100' = 'solid', 'ZF2050' = 'solid')
  ) +
  scale_shape_manual(values = c('Opt1.5C' = 4, 'ZF2100' = 2, 'ZF2050' = 21)) +
...
g_fig_2 <- ggdraw() +
  draw_plot(
    g_fig_2a + theme(legend.position = 'none'),
    x = 0,
    y = 0.05,
    width = .25,
    height = .935
  ...
  draw_plot(
    g_fig_2b + theme(legend.position = 'none'),
    x = 0.265,
    y = 0.05,
    width = .635,
    height = .935
...
ggplot2::ggsave(
  'output/revision_final/fig_2.png',
  g_fig_2,
  width = 180,
  height = 70,
  units = 'mm',
...
ggplot2::ggsave(
  'output/revision_final/fig_2.pdf',
  g_fig_2,
  width = 180,
  height = 70,
  units = 'mm',
...
df_sec_loss <- df_load %>%
  ...
  group_by(Model, Scenario, Region, Unit, Year) %>%
  complete(Variable = var_sec_flow, fill = list(value = 0)) %>%
  ungroup() %>%
  pivot_wider(names_from = Variable, values_from = value) %>%
  transmute(
    Model,
    ...
    Unit,
    Year,
    `Loss|Electricity` = `Secondary Energy|Electricity` -
      (`Secondary Energy|Hydrogen|Electricity` +
        `Secondary Energy|Hydrogen|Fossil` +
        `Secondary Energy|Hydrogen|Biomass` +
        `Final Energy|Electricity` +
        `Secondary Energy|Electricity|Own Use|DAC`),
    `Loss|Hydrogen` = `Secondary Energy|Hydrogen` -
      (`Final Energy|Liquids|Hydrogen synfuel` +
        `Final Energy|Gases|Hydrogen synfuel` +
        `Final Energy|Hydrogen`)
  ) %>%
  pivot_longer(
    cols = c('Loss|Electricity', 'Loss|Hydrogen'),
    names_to = 'Variable',
    values_to = 'value'
  )
...
for (i in c(2050, 2100)) {
  for (j in c('AIM', 'MESSAGEix')) {
    ...
    df_ele_snapshot <- df_load %>%
      ...
      mutate(
        Variable = factor(Variable, levels = rev(var_ele$Variable)),
        Scenario = factor(Scenario, levels = c('Opt1.5C', 'ZF2100', 'ZF2050'))
      )
    ...
    g_sec_flow <- tribble(
      ...
      inner_join(
        bind_rows(df_load, df_sec_loss) %>%
          filter(
            Variable %in% var_sec_flow2,
            Year == i,
            Model == j,
            Scenario %in% c('Opt1.5C', 'ZF2050')
          ) %>%
      ...
      pivot_longer(
        cols = -c(Model, Scenario, Variable, value),
        names_to = 'x',
        values_to = 'Carrier'
      ) %>%
      ...
      mutate(
        Label = ifelse(
          x == 'Hydrogen' & Carrier %in% c('Electricity'),
          ' ',
          Label
        )
      ) %>%
      mutate(
        Label = ifelse(
          x == 'Synfuel' & Carrier %in% c('Electricity', 'Hydrogen', 'Loss'),
          ' ',
          Label
        )
      ) %>%
      mutate(
        Label = ifelse(x == 'Synfuel' & Carrier == 'Loss2', 'Loss', Label)
      ) %>%
      mutate(
        Label = ifelse(x == 'Final' & Carrier == 'Synfuel', ' ', Label)
      ) %>%
      mutate(
        Alpha = ifelse(
          x == 'Hydrogen' & Carrier %in% c('Electricity'),
      ...
      mutate(
        Positionh = ifelse(x == 'Source', .2, ifelse(x == 'Final', .8, .5))
      ) %>%
      mutate(
        Positionv = ifelse(x == 'Source' & Carrier == 'Fossil', -.5, Positionv)
      ) %>%
      mutate(
        Variable = factor(
          Variable,
      ...
      geom_stratum(
        aes(fill = Carrier, alpha = Alpha),
        color = 'transparent',
        show.legend = F
      ) +
      geom_text(
        aes(label = Label, hjust = Positionh, vjust = Positionv),
        stat = 'stratum',
        size = 6.5 * 0.282
      ) +
      ...
      ylim(0, max(total$value)) +
      scale_x_discrete(
        limits = c('Source', 'Hydrogen', 'Synfuel', 'Final'),
...
df_fig_3c_1 <- df_load %>%
  ...
  group_by(Model, Scenario, Region, Year) %>%
  reframe(value = sum(value), Variable = 'Electricity\n(total)')

df_fig_3c_2 <- df_load %>%
  filter(Variable %in% c('Secondary Energy|Hydrogen')) %>%
  group_by(Model, Scenario, Region, Year) %>%
  reframe(value = sum(value), Variable = 'Hydrogen\n(total)')
...
df_fig_3c_AR6_2 <- df_load_AR6 %>%
  filter(
    Variable %in% c('Secondary Energy|Hydrogen'),
    Year %in% c(2050, 2100)
  ) %>%
  group_by(Region, Category, Case, Year) %>%
  reframe(value = sum(value)) %>%
...
g_fig_3c <- df_fig_3c %>%
  ...
  geom_crossbar(
    data = df_fig_3c_AR6 %>% mutate(Year = as.character(Year)),
    aes(x = Year, ymin = p10, ymax = p90, y = p50),
    width = .5,
    color = 'white',
    fill = '#D9D5C4FF',
  ...
  geom_text(
    data = df_fig_3c_AR6 %>% mutate(Year = as.character(Year)),
    aes(x = '2100', y = -20, label = paste0("italic(n)==", n)),
    parse = TRUE,
    size = 1.75,
    hjust = 0.8,
  ) +
...
df_fig_3d <- df_load %>%
  ...
  mutate(
    ...
    Variable = factor(
      Variable,
      levels = c('Solar PV', 'Wind', 'Storage', 'Electrolyzer')
    )
  ) %>%
  filter(
    Scenario %in% c('Opt1.5C', 'ZF2050', 'ZF2100'),
...
g_fig_3d <- df_fig_3d %>%
  ...
  geom_area(
    aes(x = Year10, y = value / 1000, fill = Scenario, group = Scenario),
    alpha = 0.1,
    position = 'identity'
  ) +
  geom_point(
  ...
  scale_shape_manual(values = c('Opt1.5C' = 4, 'ZF2100' = 2, 'ZF2050' = 21)) +
  scale_x_discrete(
    labels = c('2021-30', '', '2041-50', '', '2061-70', '', '2081-90', '')
  ) +
  labs(
    x = '',
    y = expression(paste(
      'Capacity addition (TW ',
      {
        yr^{
          -1
        }
...
l_fig_3c_1 <- g_legend(
  g_fig_3c +
    guides(color = guide_legend(ncol = 1), shape = 'none', size = 'none') +
    theme(legend.position = 'right')
)
l_fig_3c_2 <- g_legend(
  g_fig_3c + guides(color = 'none') + theme(legend.position = 'right')
)
...
g_fig_3 <- ggdraw() +
  draw_plot(
    g_fig_3d +
      theme(legend.position = 'bottom', legend.margin = margin(-15, 0, 0, 0)),
    x = .34,
    y = 0.02,
    width = .66,
    height = .53
  ...
  draw_plot(
    g_fig_3c + theme(legend.position = 'none'),
    x = 0,
    y = 0,
    width = .2,
    height = .55
  ...
  draw_plot(
    `g_ele_snapshot_2050_MESSAGEix` +
      theme(legend.position = 'none') +
      `g_sec_flow_2050_MESSAGEix` +
      plot_layout(widths = c(1, 8)),
    ,
    x = .45,
    y = 0.55,
...
ggplot2::ggsave(
  'output/revision_final/fig_3.png',
  g_fig_3,
  width = 180,
  height = 120,
  units = 'mm',
...
ggplot2::ggsave(
  'output/revision_final/fig_3.pdf',
  g_fig_3,
  width = 180,
  height = 120,
  units = 'mm',
...
df_fig_4a <- df_load %>%
  filter(
    Variable ==
      'Emissions|CO2|Energy and Industrial Processes including Direct Air Capture'
  )
...
df_fig_4a_AR6 <- df_load_AR6 %>%
  ...
  reframe(
    Variable = 'Emissions|CO2|Energy and Industrial Processes including Direct Air Capture',
    value = sum(value)
  ) %>%
  group_by(Region, Variable, Year) %>%
  reframe(
...
g_fig_4a <- df_fig_4a %>%
  ...
  geom_point(
    aes(x = Year, y = value / 1000, color = Scenario, shape = Scenario),
    size = 0.8,
    stroke = 0.4
  ) +
  geom_text(
    data = df_fig_4a_AR6 %>% filter(Year == 2050),
    aes(label = paste0('italic(n)==', n)),
    parse = TRUE,
    x = 2025,
    y = -20,
...
df_fig_4b <- df_load %>%
  ...
  transmute(
    ...
    `Emissions|CO2|Energy|Demand|Residential and Commercial`,
    `Emissions|CO2|Energy|Demand|Transportation`,
    `Emissions|CO2|Energy|Demand|Other` = `Emissions|CO2|Energy|Demand|AFOFI` +
      `Emissions|CO2|Energy|Demand|Other Sector`,
    `Emissions|CO2|Energy|Supply|Other` = `Emissions|CO2|Energy|Supply` -
      `Emissions|CO2|BECCS`,
    `Emissions|CO2|Industrial Processes`,
    `Emissions|CO2|DACCS`,
    `Emissions|CO2|BECCS`,
...
g_fig_4b <- df_fig_4b %>%
  ...
  mutate(
    Variable = factor(Variable, levels = rev(var_emi$Variable)),
    Scenario = factor(Scenario, levels = c('Opt1.5C', 'ZF2100', 'ZF2050'))
  ) %>%
  ggplot() +
  geom_hline(
  ...
  geom_point(
    data = . %>%
      ...
      reframe(value = sum(value)),
    aes(x = Scenario, y = value / 1000, shape = Variable),
    size = .9,
    color = 'black',
    fill = 'white',
    stroke = 0.4
  ...
  scale_shape_manual(
    values = c('Net CO2 (Energy + AFOLU)' = 21, 'Net CO2 (Energy)' = 25),
    labels = c(
      'Net CO2 (Energy + AFOLU)' = expression(paste(
        'Net ',
        {
          CO[2]
        },
      ...
      'Net CO2 (Energy)' = expression(paste(
        'Net ',
        {
          CO[2]
        },
...
l_fig_4bc_1 <- g_legend(
  g_fig_4b + guides(fill = guide_legend(nrow = 2), shape = 'none')
)
l_fig_4bc_2 <- g_legend(
  g_fig_4b + guides(fill = 'none', shape = guide_legend(nrow = 2))
)

g_fig_4 <- ggdraw() +
  draw_plot(
    g_fig_4a + theme(legend.position = 'none'),
    x = 0,
    y = .15,
    width = .45,
    height = .8
  ...
  draw_plot(
    g_fig_4b + theme(legend.position = 'none'),
    x = .575,
    y = .15,
    width = .2,
    height = .8
  ...
  draw_plot(
    g_fig_4c + theme(legend.position = 'none'),
    x = .785,
    y = .15,
    width = .2,
    height = .8
...
ggplot2::ggsave(
  'output/revision_final/fig_4.png',
  g_fig_4,
  width = 180,
  height = 65,
  units = 'mm',
...
ggplot2::ggsave(
  'output/revision_final/fig_4.pdf',
  g_fig_4,
  width = 180,
  height = 65,
  units = 'mm',
...
g_fig_5a <- df_fig_5a %>%
  ...
  geom_crossbar(
    data = df_fig_5a_minmax,
    aes(x = Model, ymin = vmin, ymax = vmax, y = vmax, fill = Model),
    width = .7,
    color = 'black',
    fatten = 0,
    linewidth = 0.15,
  ...
  geom_point(
    aes(x = Model, y = value, shape = Scenario, size = Scenario),
    stroke = 0.3,
    size = 1,
    color = 'black'
  ) +
  labs(
    x = '',
    y = '',
    title = 'Solar and wind power capacity addition in the 2020-2050 period (% change from Opt1.5C)'
  ) +
  scale_fill_manual(
    values = c('AIM' = '#DD4477FF', 'MESSAGEix' = '#0099C6FF')
  ) +
  ...
  scale_y_continuous(
    ...
    limits = c(-fig_5a_max$value, fig_5a_max$value)
  ) +
  coord_flip() +
...
df_fig_5b <- df_load %>%
  ...
  filter(Year >= 2021) %>%
  mutate(
    discount = exp(-(Year - 2021) * log(1 + 0.05)),
...
g_fig_5b <- df_fig_5b %>%
  ...
  labs(
    x = '',
    y = '',
    title = 'Investment in energy supply sectors in the 2020-2100 period (% change from Opt1.5C)'
  ) +
  scale_fill_manual(
    values = c('AIM' = '#DD4477FF', 'MESSAGEix' = '#0099C6FF')
  ) +
  ...
  scale_y_continuous(
    labels = scales::percent,
    limits = c(-fig_5b_max$value, fig_5b_max$value)
  ) +
  coord_flip() +
  plot_theme_white_rect +
  theme(
...
g_fig_5c <- df_fig_5c %>%
  ...
  labs(
    x = '',
    y = '',
    title = 'Stranded capacity of coal power plants in the 2020-2050 period (% change from Opt1.5C)'
  ) +
  scale_fill_manual(
    values = c('AIM' = '#DD4477FF', 'MESSAGEix' = '#0099C6FF')
  ) +
  ...
  scale_y_continuous(
    labels = scales::percent,
    limits = c(-fig_5c_max$value, fig_5c_max$value)
  ) +
  coord_flip() +
  plot_theme_white_rect +
  theme(
...
df_fig_5d <- df_load %>%
  mutate(Model = factor(Model, levels = c('MESSAGEix', 'AIM'))) %>%
  filter(
    Variable %in% c('Investment|Energy Demand|Efficiency and Decarbonization')
  ) %>%
  group_by(Model, Scenario, Region, Variable, Unit) %>%
  complete(Year = seq(2020, 2100)) %>%
  mutate(value = na_interpolation(value)) %>%
...
g_fig_5d <- df_fig_5d %>%
  ...
  labs(
    x = '',
    y = '',
    title = 'Investment in energy demand sectors in the 2020-2100 period (% change from Opt1.5C)'
  ) +
  scale_fill_manual(
    values = c('AIM' = '#DD4477FF', 'MESSAGEix' = '#0099C6FF')
  ) +
  ...
  scale_y_continuous(
    labels = scales::percent,
    limits = c(-fig_5d_max$value, fig_5d_max$value)
  ) +
  coord_flip() +
  plot_theme_white_rect +
  theme(
...
g_fig_5e <- df_fig_5e %>%
  ...
  geom_point(
    aes(x = Model, y = value, shape = Scenario),
    stroke = 0.3,
    size = 1,
    color = 'black'
  ) +
  labs(
    x = '',
    y = '',
    title = 'Non-hydrocarbon final energy in the 2020-2050 period (% change from Opt1.5C)'
  ) +
  scale_fill_manual(
    values = c('AIM' = '#DD4477FF', 'MESSAGEix' = '#0099C6FF')
  ) +
  ...
  scale_y_continuous(
    labels = scales::percent,
    limits = c(-fig_5e_max$value, fig_5e_max$value)
  ) +
  coord_flip() +
  plot_theme_white_rect +
  theme(
...
g_fig_5f <- df_fig_5f %>%
  ...
  labs(
    ...
    title = expression(paste(
      ...
      ' storage in the 2020-2100 period (% change from Opt1.5C)'
    ))
  ) +
  scale_fill_manual(
    values = c('AIM' = '#DD4477FF', 'MESSAGEix' = '#0099C6FF')
  ...
  scale_y_continuous(
    labels = scales::percent,
    limits = c(-fig_5f_max$value, fig_5f_max$value)
  ) +
  coord_flip() +
  plot_theme_white_rect +
  theme(
...
df_fig_5g <- df_load %>%
  ...
  filter(
    Variable %in%
      c(
        'CCUS|Geological Storage|Biomass',
...
g_fig_5g <- df_fig_5g %>%
  ...
  labs(
    ...
    y = '',
    title = expression(paste(
      'CDR in the 2020-2100 period (% change from Opt1.5C)'
    ))
  ) +
  scale_fill_manual(
    values = c('AIM' = '#DD4477FF', 'MESSAGEix' = '#0099C6FF')
  ...
  scale_y_continuous(
    labels = scales::percent,
    limits = c(-fig_5g_max$value, fig_5g_max$value)
  ) +
  coord_flip() +
  plot_theme_white_rect +
  theme(
...
g_fig_5h <- df_fig_5h %>%
  ...
  labs(
    ...
    y = '',
    title = expression(paste(
      'Biomass primary energy in the 2020-2100 period (% change from Opt1.5C)'
    ))
  ) +
  scale_fill_manual(
    values = c('AIM' = '#DD4477FF', 'MESSAGEix' = '#0099C6FF')
  ...
  scale_y_continuous(
    labels = scales::percent,
    limits = c(-fig_5h_max$value, fig_5h_max$value)
  ) +
  coord_flip() +
  plot_theme_white_rect +
  theme(
...
g_fig_5i <- df_fig_5i %>%
  ...
  labs(
    ...
    title = expression(paste(
      ...
      ' emission from AFOLU sector in the 2020-2100 period (% change from Opt1.5C)'
    ))
  ) +
  scale_fill_manual(
    values = c('AIM' = '#DD4477FF', 'MESSAGEix' = '#0099C6FF')
  ...
  scale_y_continuous(
    labels = scales::percent,
    limits = c(-fig_5i_max$value, fig_5i_max$value)
  ) +
  coord_flip() +
  plot_theme_white_rect +
  theme(
...
g_fig_5 <- ggdraw() +
  ...
  draw_plot(
    g_fig_5d +
      ...
      theme(
        panel.background = element_rect(color = NA, fill = 'white'),
        plot.background = element_rect(color = NA, fill = '#f2daef')
      ),
    x = 0,
    y = 0.58,
    width = 1,
  ...
  draw_plot(l_fig_5_1, x = 0, y = 0.03, width = 1, height = .044) +
  draw_plot(l_fig_5_2, x = 0.1, y = 0, width = 0.8, height = .044) +
  draw_plot(header_fig_5, x = 0.005, y = 0.96, width = 1, height = .04) +
  draw_text('Energy supply', angle = 90, size = 6.5, x = 0.015, y = 0.8) +
  draw_text('Energy demand', angle = 90, size = 6.5, x = 0.015, y = 0.565) +
  draw_text('CCS', angle = 90, size = 6.5, x = 0.015, y = 0.375) +
  draw_text('Land use', angle = 90, size = 6.5, x = 0.015, y = 0.18) +
  draw_plot_label(
    label = c('a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i'),
    x = c(0.045, 0.045, 0.045, 0.045, 0.045, 0.055, 0.045, 0.045, 0.055),
    y = c(0.96, 0.86, 0.76, 0.66, 0.58, 0.48, 0.38, 0.28, 0.18),
    size = 7
  ) +
  theme(panel.background = element_rect(fill = 'white', colour = NA))
plot(g_fig_5)
...
ggplot2::ggsave(
  'output/revision_final/fig_5.png',
  g_fig_5,
  width = 100,
  height = 140,
  units = 'mm',
...
ggplot2::ggsave(
  'output/revision_final/fig_5.pdf',
  g_fig_5,
  width = 100,
  height = 140,
  units = 'mm',
...
g_fig_S1a <- df_fig_1a %>%
  ...
  geom_mask() +
  geom_point(
    data = df_fig_1a_AR6 %>% filter(Year == 2100, Category %in% c('C1', 'C2')),
    color = '#D9D5C4FF',
    size = 0.7,
    shape = 4,
    stroke = 0.4
  ...
  annotate(
    ...
    label = paste0(
      "'C1: ' * italic(n)==",
      df_fig_1a_AR6 %>%
        filter(Year == 2100, Category %in% c('C1')) %>%
        nrow()
    ),
    parse = TRUE,
    size = 1.75
  ...
  annotate(
    ...
    label = paste0(
      "'C2: ' * italic(n)==",
      df_fig_1a_AR6 %>%
        filter(Year == 2100, Category %in% c('C2')) %>%
        nrow()
    ),
    parse = TRUE,
    size = 1.75
...
g_fig_S1b <- df_fig_1b %>%
  ...
  geom_mask() +
  geom_point(
    data = df_fig_1b_AR6 %>% filter(Year == 2100, Category %in% c('C1', 'C2')),
    color = '#D9D5C4FF',
    size = 0.7,
    shape = 4,
    stroke = 0.4
...
g_fig_S1c <- df_fig_1c %>%
  ...
  geom_mask() +
  geom_point(
    data = df_fig_1c_AR6 %>% filter(Year == 2100, Category %in% c('C1', 'C2')),
    color = '#D9D5C4FF',
    size = 0.7,
    shape = 4,
    stroke = 0.4
...
g_fig_S1d <- df_fig_S1d %>%
  ...
  annotate(
    ...
    label = paste0(
      "italic(n)==",
      (df_fig_S1d_AR6 %>% filter(Year == 2050) %>% pull(n))[1]
    ),
    parse = TRUE,
    size = 1.75
  ) +
...
df_fig_S1e_AR6 <- df_load_AR6 %>%
  filter(
    Year %in% c(2030, 2050, 2070, 2100),
    Variable %in% c('Primary Energy|Oil')
  ) %>%
  group_by(Region, Category, Case, Year) %>%
  reframe(value = sum(value)) %>%
  group_by(Region, Year) %>%
...
g_fig_S1e <- df_fig_S1e %>%
  ...
  annotate(
    ...
    label = paste0(
      "italic(n)==",
      (df_fig_S1e_AR6 %>% filter(Year == 2050) %>% pull(n))[1]
    ),
    parse = TRUE,
    size = 1.75
  ) +
...
df_fig_S1f_AR6 <- df_load_AR6 %>%
  filter(
    Year %in% c(2030, 2050, 2070, 2100),
    Variable %in% c('Primary Energy|Gas')
  ) %>%
  group_by(Region, Category, Case, Year) %>%
  reframe(value = sum(value)) %>%
  group_by(Region, Year) %>%
...
g_fig_S1f <- df_fig_S1f %>%
  ...
  annotate(
    ...
    label = paste0(
      "italic(n)==",
      (df_fig_S1f_AR6 %>% filter(Year == 2050) %>% pull(n))[1]
    ),
    parse = TRUE,
    size = 1.75
  ) +
...
ggplot2::ggsave(
  'output/revision_final/fig_S1.png',
  g_fig_S1,
  width = 180,
  height = 105,
  units = 'mm',
...
g_fig_S2a <- df_fig_S2a %>%
  mutate(
    Variable = factor(Variable, levels = rev(var_ele2$Variable)),
    Scenario = factor(
      Scenario,
      levels = c(
        'Opt1.5C',
...
g_fig_S2b <- df_fig_S2b %>%
  mutate(
    Variable = factor(Variable, levels = rev(var_hyd2$Variable)),
    Scenario = factor(
      Scenario,
      levels = c(
        'Opt1.5C',
...
g_fig_S2 <- g_fig_S2a + g_fig_S2b + plot_layout(ncol = 1)

ggplot2::ggsave(
  'output/revision_final/fig_S2.png',
  g_fig_S2,
  width = 180,
  height = 120,
  units = 'mm',
...
df_fig_S3b <- df_load %>%
  ...
  mutate(
    Variable = case_when(
      Variable %in%
        c('Final Energy|Industry|Solids|Coal') ~ 'Final Energy|Solids|Coal',
      Variable %in%
        c(
          'Final Energy|Industry|Liquids|Fossil'
        ) ~ 'Final Energy|Liquids|Fossil',
      Variable %in%
        c('Final Energy|Industry|Gases|Fossil') ~ 'Final Energy|Gases|Fossil',
      Variable %in%
        c(
          'Final Energy|Industry|Solids|Biomass',
          'Final Energy|Industry|Liquids|Biomass',
      ...
      Variable %in%
        c(
          'Final Energy|Industry|Gases|Hydrogen synfuel',
          'Final Energy|Industry|Liquids|Hydrogen synfuel'
        ) ~ 'Final Energy|Hydrogen synfuel'
    )
  )

g_fig_S3b <- df_fig_S3b %>%
  filter(Scenario %in% c('Opt1.5C', 'ZF2100', 'ZF2050')) %>%
  mutate(
    Variable = factor(Variable, levels = rev(var_fin_agg$Variable)),
    Scenario = factor(
...
df_fig_S3c <- df_load %>%
  ...
  mutate(
    Variable = case_when(
      ...
      Variable ==
        'Final Energy|Residential and Commercial|Solar' ~ 'Final Energy|Solar',
      Variable ==
      ...
      Variable ==
        'Final Energy|Residential and Commercial|Hydrogen' ~ 'Final Energy|Hydrogen',
      Variable %in%
        c(
          'Final Energy|Residential and Commercial|Gases|Hydrogen synfuel',
          'Final Energy|Residential and Commercial|Liquids|Hydrogen synfuel'
...
df_fig_S3d <- df_load %>%
  ...
  mutate(
    Variable = case_when(
      ...
      Variable ==
        'Final Energy|Transportation|Hydrogen' ~ 'Final Energy|Hydrogen',
      Variable %in%
        c(
          'Final Energy|Transportation|Gases|Hydrogen synfuel',
          'Final Energy|Transportation|Liquids|Hydrogen synfuel'
...
l_fig_S3 <- g_legend(g_fig_S3a)

g_fig_S3 <- (((g_fig_S3a +
  labs(title = 'Total', tag = 'a') +
  theme(legend.position = 'none') +
  guides(fill = guide_legend(nrow = 1))) +
  plot_spacer() +
  l_fig_S3 +
  plot_layout(widths = c(9, 0, 1))) /
  ((g_fig_S3b +
    labs(title = 'Industry', tag = 'b') +
    theme(legend.position = 'none') +
    guides(fill = guide_legend(nrow = 1))) +
    (g_fig_S3c +
      labs(title = 'Buidings', tag = 'c') +
      theme(legend.position = 'none') +
      guides(fill = guide_legend(nrow = 1))) +
    (g_fig_S3d +
      labs(title = 'Transport', tag = 'd') +
      theme(legend.position = 'none') +
      guides(fill = guide_legend(nrow = 1))))) +
...
ggplot2::ggsave(
  'output/revision_final/fig_S3.png',
  g_fig_S3,
  width = 180,
  height = 110,
  units = 'mm',
...
g_fig_S4a <- df_fig_S4 %>%
  ...
  geom_text(
    data = df_fig_S4_AR6 %>% filter(Year == 2050),
    aes(label = paste0("italic(n)==", n)),
    parse = TRUE,
    x = 2025,
    y = -20,
...
g_fig_S4 <- ggdraw() +
  draw_plot(
    g_fig_S4a + theme(legend.position = 'none'),
    x = 0,
    y = 0,
    width = .85,
    height = 1
...
ggplot2::ggsave(
  'output/revision_final/fig_S4.png',
  g_fig_S4,
  width = 130,
  height = 65,
  units = 'mm',
...
g_fig_S5 <- ggdraw() +
  ...
  draw_plot(
    `g_ele_snapshot_2100_MESSAGEix` +
      theme(legend.position = 'none') +
      `g_sec_flow_2100_MESSAGEix` +
      plot_layout(widths = c(1, 8)),
    ,
    x = .45,
    y = 0,
...
plot(g_fig_S5)
ggplot2::ggsave(
  'output/revision_final/fig_S5.png',
  g_fig_S5,
  width = 180,
  height = 60,
  units = 'mm',
...
g_fig_S6a <- df_fig_4b %>%
  filter(
    Model == 'AIM' &
      Year %in% seq(2020, 2100, 5) |
      Model == 'MESSAGEix' &
        Year %in% c(seq(2020, 2055, 5), seq(2060, 2100, 10))
  ) %>%
  mutate(
    Variable = factor(Variable, levels = rev(var_emi$Variable)),
    Scenario = factor(
...
df_fig_S6b_1 <- df_load %>%
  ...
  mutate(
    Variable = paste0(
      str_remove_all(Variable, 'CCUS\\|Geological Storage\\|'),
      ' (CCS)'
    )
  )

df_fig_S6b_2 <- df_load %>%
  filter(
    Variable %in%
      c(
        'CCUS|Utilization|Energy|Biomass',
  ...
  mutate(
    Variable = paste0(
      str_remove_all(Variable, 'CCUS\\|Utilization\\|Energy\\|'),
      ' (CCU)'
    )
  )

df_fig_S6b <- bind_rows(df_fig_S6b_1, df_fig_S6b_2)

g_fig_S6b <- df_fig_S6b %>%
  filter(
    Model == 'AIM' &
      Year %in% seq(2020, 2100, 5) |
      Model == 'MESSAGEix' &
  ...
  pivot_longer(
    cols = -c(Model, Region, Scenario, Unit, Year),
    names_to = 'Variable',
    values_to = 'value'
  ) %>%
  mutate(
...
g_figS6 <- g_fig_S6a +
  ...
  labs(tag = 'b') +
  l_fig_S6b +
  plot_layout(ncol = 1, guides = 'collect', heights = c(5, .2, 0, 3, .4)) &
  theme(legend.position = 'none')
ggplot2::ggsave(
  'output/revision_final/fig_S6.png',
  g_figS6,
  width = 180,
  height = 150,
  units = 'mm',
...
mineral_intensity <- read_xlsx(
  'data/minerals/Supplementary Materials for Navigating energy transition solutions for climate targets with minerals constraint.xlsx',
  'S5. Mineral_intensity'
) %>%
  drop_na(Value) %>%
  transmute(
    Category = `Category technology`,
    Subdivision = case_when(
      `Category technology` == 'wind' ~ 'wind',
      TRUE ~ `Subdivision technology`
    ),
    Metal = `Symbol `,
...
market_share <- read_xlsx(
  ...
  mutate(
    Category = case_when(
      Category == 'Hydrogen by Electricity' ~ 'Electric hydrogen',
...
df_mineral <- df_load %>%
  mutate(
    Value = ifelse(
      Variable == 'Capacity Additions|Electricity|Storage Capacity',
      value / 6.2113,
      value
    )
  ) %>%
...
df_fig_S7 <- df_mineral %>%
  ...
  complete(Year = seq(2020, 2100)) %>%
  mutate(
    Min = na_locf(Min, option = 'nocb'),
    Mean = na_locf(Mean, option = 'nocb'),
    Max = na_locf(Max, option = 'nocb')
  ) %>%
  group_by(Model, Scenario, Metal) %>%
  mutate(Min = cumsum(Min), Mean = cumsum(Mean), Max = cumsum(Max)) %>%
  filter(
    Year %in%
      seq(2020, 2055, 5) |
      Year %in% seq(2060, 2100, 5) & Model == 'AIM' |
      Year %in% seq(2060, 2100, 10) & Model == 'MESSAGEix'
  )
...
ggplot2::ggsave(
  'output/revision_final/fig_S7.png',
  g_fig_S7,
  width = 180,
  height = 100,
  units = 'mm',
...
df_fig_S8 <- df_load %>%
  ...
  mutate(
    Variable = str_remove_all(Variable, 'Trade\\||\\|Volume'),
    Variable = str_remove_all(Variable, '.*\\|'),
    Variable = case_when(
      Variable %in% c('Hydrogen', 'Ammonia') ~ 'Hydrogen',
      TRUE ~ Variable
    )
  ) %>%
...
ggplot2::ggsave(
  'output/revision_final/fig_S8.png',
  g_fig_S8,
  width = 180,
  height = 75,
  units = 'mm',
...
g_fig_S9a <- df_load_w_MACRO %>%
  filter(Variable %in% c('Final Energy'), Model != 'AIM') %>%
  pivot_wider(names_from = Scenario, values_from = value) %>%
  mutate(across(
    c(Opt1.5C, ZF2050, ZF2060, ZF2070, ZF2080, ZF2090, ZF2100),
    ~ . / Opt1.5C
  )) %>%
  select(-Opt1.5C) %>%
  pivot_longer(
    cols = c(ZF2050, ZF2060, ZF2070, ZF2080, ZF2090, ZF2100),
    names_to = 'Scenario',
    values_to = 'value'
  ) %>%
  ggplot() +
  ...
  labs(
    x = '',
    y = expression(paste('Change in total final consumption (%)')),
    tag = 'a'
  ) +
  facet_wrap(vars(Model)) +
  plot_theme_white +
...
g_fig_S9 <- g_fig_S9a + g_fig_S9b + plot_layout(widths = c(1, 1))
plot(g_fig_S9)

ggplot2::ggsave(
  'output/revision_final/fig_S9.png',
  g_fig_S9,
  width = 180,
  height = 80,
  units = 'mm',
